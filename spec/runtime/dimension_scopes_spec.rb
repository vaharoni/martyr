...
describe 'Dimension Scopes Loading and Tree Traversal' do
  ...
  describe 'fetching values up' do
    it 'returns the right record when using #recursive_lookup_up on same query level' do
      record = invoice_line_level.recursive_lookup_up(1, level: invoice_line_level)
      expect(record).to eq(InvoiceLine.find(1))
    end

    it 'returns the right record when using #recursive_lookup_up on query level above' do
      record = invoice_line_level.recursive_lookup_up(1, level: invoice_level)
      invoice_line = InvoiceLine.find(1)
      expect(record).to eq(invoice_line.invoice)
    end

    it 'returns the right record when using #recursive_lookup_up on direct degenerate level 1 level above' do
      record = invoice_level.recursive_lookup_up(1, level: city_level)
      expect(record).to eq(Invoice.find(1).billing_city)
    end

    it 'returns the right record when using #recursive_lookup_up on direct degenerate level 3 levels above' do
      record = invoice_level.recursive_lookup_up(1, level: country_level)
      expect(record).to eq(Invoice.find(1).billing_country)
    end

    it 'returns the right record when using #recursive_lookup_up on degenerate level above another query level' do
      record = invoice_line_level.recursive_lookup_up(1, level: country_level)
      invoice_line = InvoiceLine.find(1)
      expect(record).to eq(invoice_line.invoice.billing_country)
    end

    it 'returns the same value when using #recursive_lookup_up on same degenerate level' do
      value = city_level.recursive_lookup_up('Boston', level: city_level)
      expect(value).to eq('Boston')
    end

    it 'returns the right record when using #recursive_lookup_up on degenerate level above degenerate level' do
      record = city_level.recursive_lookup_up('Boston', level: country_level)
      expect(record).to eq('USA')
    end
  end

  describe 'fetching values down' do
    it 'returns the right record when using #recursive_lookup_down on same query level' do
      records = invoice_level.recursive_lookup_down('invoice-1', level: invoice_level)
      expect(records).to eq([Invoice.find(1)])
    end

    it 'returns right records when using #recursive_lookup_down on same query level with multiple records' do
      records = invoice_level.recursive_lookup_down(['invoice-1', 'invoice-2'], level: invoice_level)
      expect(records).to eq([Invoice.find(1), Invoice.find(2)])
    end

    it 'returns the right record when using #recursive_lookup_down on query level below from query level' do
      records = invoice_level.recursive_lookup_down('invoice-1', level: invoice_line_level)
      invoice_lines = InvoiceLine.where(invoice_id: 1)
      expect(records).to eq(invoice_lines)
    end

    it 'returns the right record when using #recursive_lookup_down on query level below from degenerate level 1 level above' do
      records = city_level.recursive_lookup_down('Boston', level: invoice_level).sort_by(&:id)
      invoices = Invoice.where(billing_city: 'Boston').to_a.sort_by(&:id)
      expect(records).to eq(invoices)
    end

    it 'returns the right record when using #recursive_lookup_down on query level below from degenerate level 1 level above for multiple values' do
      records = city_level.recursive_lookup_down(['Boston', 'New York'], level: invoice_level).sort_by(&:id)
      invoices = Invoice.where(billing_city: ['Boston', 'New York']).to_a.sort_by(&:id)
      expect(records).to eq(invoices)
    end

    it 'returns the right record when using #recursive_lookup_down on query level below from degenerate level 3 levels above' do
      records = country_level.recursive_lookup_down('USA', level: invoice_level).sort_by(&:id)
      invoices = Invoice.where(billing_country: 'USA').to_a.sort_by(&:id)
      expect(records).to eq(invoices)
    end

    it 'returns the right record when using #recursive_lookup_down on query level below another query level from degenerate level' do
      records = country_level.recursive_lookup_down('USA', level: invoice_line_level).sort_by(&:id)
      invoice_lines = Invoice.where(billing_country: 'USA').flat_map(&:invoice_lines).sort_by(&:id)
      expect(records).to eq(invoice_lines)
    end

    it 'returns the right record when using #recursive_lookup_down on same degenerate level' do
      records = city_level.recursive_lookup_down('Boston', level: city_level)
      expect(records).to eq(['Boston'])
    end

    it 'returns the right record when using #recursive_lookup_down on degenerate level below degenerate level' do
      records = country_level.recursive_lookup_down('USA', level: city_level).sort
